...
static pthread_mutex_t lock_write;
...
int main()
{
    ...
    if (0 != pthread_mutex_init(&lock_write, NULL))
    {
        printf("\n mutex init failed\n");
        exit(1);
    }

    if (0 != pthread_mutex_init(&lock_read, NULL))
    {
        printf("\n mutex init failed\n");
        exit(1);
    }
...
void *CosumerFunc(void *param)
{
    ...
    while (1)
    {
        ...
        if (0 != pthread_mutex_lock(&lock_read))
        {
        ...
        if (0 != pthread_mutex_unlock(&lock_read))
        {
...
void *ProducerFunc(void *param)
{
    ...
    for (i = 0; i < MESSAGES; ++i)
    {
        ...
        if (0 != pthread_mutex_lock(&lock_write))
        {
            fprintf(stderr, "Error - pthread_mutex_lock()\n");
            exit(1);
        }
        ...
        if (0 != pthread_mutex_unlock(&lock_write))
        {
            fprintf(stderr, "Error - pthread_mutex_unlock()\n");
            exit(1);
        }
